...
在上面的例子中，只要列表中的任意一个角色有访问按钮组件的权限，就会渲染按钮组件，否则什么也不做。这段代码看起来似乎没什么大问题。但如果此时要新增一个角色，试想会发生什么？我们可能需要在上百个地方去配置新增角色。虽然给项目造成了很大开销，不过好在满足了业务需求。但如果需求是根据动态生成的角色去控制组件的显示与否，又该如何解决呢？比如管理员可以动态创建一个新的角色，那么这个新的角色同样应该参与按钮组件显隐的控制。但是由于按钮组件的显示与否是根据「固定」的角色列表来判定的，因此无法满足「动态」增加或删除角色的需求。



问题的关键就在于：RBAC 权限模型中，角色不是固定的而是动态变化的。我们可以随时增加或者修改一个角色。因此不能将组件的权限控制和角色绑定到一起。
